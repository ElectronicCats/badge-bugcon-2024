...
const uint8_t uuid_zeros[ESP_UUID_LEN_128] = {
...
bool esp_ble_is_ibeacon_packet(uint8_t* adv_data, uint8_t adv_data_len) {
  ...
  if ((adv_data != NULL) && (adv_data_len == 0x1E)) {
    if (!memcmp(adv_data, (uint8_t*) &ibeacon_common_head,
...
esp_err_t esp_ble_config_ibeacon_data(esp_ble_ibeacon_vendor_t* vendor_config,
  ...
  if ((vendor_config == NULL) || (ibeacon_adv_data == NULL) ||
      (!memcmp(vendor_config->proximity_uuid, uuid_zeros,
               sizeof(uuid_zeros)))) {
    return ESP_ERR_INVALID_ARG;
  }

  memcpy(&ibeacon_adv_data->ibeacon_head, &ibeacon_common_head,
         sizeof(esp_ble_ibeacon_head_t));
  memcpy(&ibeacon_adv_data->ibeacon_vendor, vendor_config,
         sizeof(esp_ble_ibeacon_vendor_t));
